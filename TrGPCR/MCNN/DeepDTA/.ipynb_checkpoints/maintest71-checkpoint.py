...
from modeltest111 import DTA, test
print(sys.argv)

# ↓↓↓↓↓↓↓↓↓↓↓↓↓↓
# 显示进度条
SHOW_PROCESS_BAR = True
data_path = r'/2211041012/MCNN/MCNN/DeepDTA/test71'
...
path = Path(f'../runs/DeepDTA_{datetime.now().strftime("%Y%m%d%H%M%S")}_{seed}')
device = torch.device("cuda:0")  # or torch.device('cpu')
# device = torch.device('cpu')

max_seq_len = 1000  # 蛋白质序列长度
...
def training(model, training_loader, loss_function, optim):
    ...
    model.train()
    n = 0
    for _, (seq, smile, y) in enumerate(training_loader):   #, pkt
        for seq, smile, y in training_loader:              #, pkt
            seq = seq.to(device)
...
data_loaders = {phase_name:
                    DataLoader(MyDataset(data_path, phase_name, max_seq_len=max_seq_len,
                                          max_smi_len=max_smi_len),     #max_pkt_len=max_pkt_len,
